...
void print_help(const char* application) {
    std::cout
        ...
        << "  FEEDER_CONFIGURATION      Feeder configuration for mapping service to vss\n"
...
int main(int argc, char** argv) {
    ...
    {
        std::cout<<"JSON Config Service ID " <<std::hex << mapping.metaData.serviceID <<std::endl;
        std::cout<<"JSON Config VSS Node Info "<<mapping.vssNodes.size() <<std::endl;
        for (const auto& node : mapping.vssNodes)
        {
            std::cout << "Name: " << node.name << ",Data type " <<node.datatype <<", Byte Offset: " << unsigned(node.byteOffset) << ", Length: " << unsigned(node.length) << std::endl;
        }
    }
    ...
    adapter.InitDataBrokerFeeder(target_str, token_str,config);
