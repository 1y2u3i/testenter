...
public:
    ...
    bool InitDataBrokerFeeder(const std::string &databroker_addr, const std::string& auth_token, sdv::someip::SomeIPConfig _config);
    bool InitSomeipClient(sdv::someip::SomeIPConfig _config);
    sdv::databroker::v1::DataType getDataType(std::string dataType);
    void copyJsonValueType(uint8_t* payload, const Json::Value value, size_t size);
...
protected:
    ...
    void on_actuator_change(sdv::broker_feeder::kuksa::ActuatorValues datapoints, sdv::someip::SomeIPConfig _config);
