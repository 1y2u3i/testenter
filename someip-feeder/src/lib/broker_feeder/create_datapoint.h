...
sdv::databroker::v1::Datapoint createDatapoint(const Json::Value& value) {
    sdv::databroker::v1::Datapoint datapoint;
    if (value.isBool()) 
    {
        datapoint.set_bool_value(value.asBool());
    }
    else if(value.isString())
    {
        datapoint.set_string_value(value.asString());
    }
    else if(value.isNumeric())
    {
        if (value.isInt()) 
        {
            datapoint.set_int32_value(value.asInt());
        }
        else if (value.isUInt())
        {
            datapoint.set_uint32_value(value.asUInt());
        }
        else if (value.isInt64())
        {
            datapoint.set_int64_value(value.asInt64());
        }
        else if (value.isUInt64())
        {
            datapoint.set_uint64_value(value.asUInt64());
        } 
        else if (value.isDouble()) 
        {
            datapoint.set_double_value(value.asDouble());
        }
    }
    return datapoint;
}
