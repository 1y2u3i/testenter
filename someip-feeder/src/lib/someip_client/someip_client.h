...
struct VSSNode {
    ...
    uint8_t length;
    uint16_t methodId;
    std::map<Json::Value, Json::Value> transformMapping;
};

struct SOMEIPConfigdata {
...
struct VSSConfigInfo {
    SOMEIPConfigdata metaData;
    std::vector<VSSNode> vssNodes;
};
...
struct SomeIPConfig {
    ...
    std::vector<VSSConfigInfo>  VSSMap;
