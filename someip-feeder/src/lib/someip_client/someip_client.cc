...
void SomeIPClient::init_event_service(
    ...
    LOG_INFO << "Request event ["
            ...
            << std::endl;
/* Register for all events upto 0x8021 */			
    for (int i = 0; i <= 33; i++)
    {        
        app_->request_event(
                service, instance, (event + i),
                its_groups,
                event_type,
                reliability_type);
        ...
        LOG_INFO << "Subscribing ["
            ...
            << ", event:0x"
            << std::hex << std::setfill('0') << std::setw(4) << (event + i)
            << std::endl;

        app_->subscribe(service, instance, event_group, service_major, (event + i));
    }
}
...
void SomeIPClient::on_availability(vsomeip::service_t _service, vsomeip::instance_t _instance, bool _is_available) {
    ...
    if (_service == service_ && _instance == instance_ && _is_available == false) 
    {
        LOG_INFO << "Service ["
            << std::setw(4) << std::setfill('0') << std::hex << _service << "."
            << std::setw(4) << std::setfill('0') << std::hex << _instance
    ...
    if (_service == service_ && _instance == instance_ && _is_available == true) 
    {
        init_event_service(service_, instance_, event_group_, event_, service_major_, service_minor_);
    }
...
SomeIPConfig SomeIPClient::createEnvConfig() {
    ...
    config.feeder_json_config = getEnvironmentStr("FEEDER_CONFIGURATION", "");
    config.app_name =  getEnvironmentStr("VSOMEIP_APPLICATION_NAME", "UNKNOWN");

    // req/response support enabled
    config.req.use_req = 1;

    getjsonConfig(config.feeder_json_config,config.VSSMap);

    for (const auto& mapping : config.VSSMap)
    {
        if(mapping.metaData.serviceType.compare("Event") == 0)
        {
            config.service = mapping.metaData.serviceID;
            config.instance = mapping.metaData.instanceID;
            config.event_group = mapping.metaData.EventGroup;
            config.event = mapping.metaData.EventID;
            config.service_major = mapping.metaData.majorVersion;
            config.service_minor = mapping.metaData.minorVersion;

        }
        else if (mapping.metaData.serviceType.compare("Method") == 0)
        {
            config.req.service = mapping.metaData.serviceID;
            config.req.instance = mapping.metaData.instanceID;
            config.req.service_major = mapping.metaData.majorVersion;
            config.req.service_minor = mapping.metaData.minorVersion;
            config.req.method = mapping.metaData.methodID;
        }
    }
    std::cout << "config.service : " << std::setw(4) << std::hex <<config.service << std::endl;
    std::cout << "config.instance : " << std::setw(4) << std::hex << config.instance  << std::endl;
    std::cout << "config.event_group : " << std::setw(4) << std::hex << config.event_group << std::endl;
    std::cout << "config.event : " << std::setw(4) << std::hex << config.event << std::endl;
    std::cout << "config.service_major : " << std::hex << unsigned(config.service_major) << std::endl;
    std::cout << "config.service_minor : " << std::hex << unsigned(config.service_minor) << std::endl;

    std::cout << "config.req.service : " << std::setw(4) << std::hex <<config.req.service << std::endl;
    std::cout << "config.req.instance : " << std::setw(4) << std::hex << config.req.instance  << std::endl;
    std::cout << "config.req.service_major : " << std::hex << unsigned(config.req.service_major) << std::endl;
    std::cout << "config.req.service_minor : " << std::hex << unsigned(config.req.service_minor) << std::endl;
    std::cout << "config.req.method : " << std::setw(4) << std::hex << config.req.method << std::endl;
     return config;
}
...
int getjsonConfig(std::string filename, std::vector<VSSConfigInfo>& vssInfo) {
    ...
    const auto& someipMappings = root["SomeipMapping"];
    for (const auto& someipMapping : someipMappings) {
        VSSConfigInfo configinfo;

        configinfo.metaData.serviceType = someipMapping["ServiceType"].asString();
        configinfo.metaData.serviceID = someipMapping["ServiceID"].asUInt();
        configinfo.metaData.instanceID = someipMapping["InstanceID"].asUInt();
        configinfo.metaData.methodID = someipMapping.get("MethodID", 0).asUInt();
        configinfo.metaData.EventID = someipMapping.get("EventID", 0).asUInt();
        configinfo.metaData.EventGroup = someipMapping.get("EventGroup", 0).asUInt();
        configinfo.metaData.majorVersion = someipMapping["MAJOR_VERSION"].asUInt();
        configinfo.metaData.minorVersion = someipMapping["MINOR_VERSION"].asUInt();

        const auto& vssList = someipMapping["VSSNode"];
        for (const auto& node : vssList) {
            VSSNode vssNode;
            vssNode.name = node["Name"].asString();
            vssNode.byteOffset = node["ByteOffset"].asUInt64();
            vssNode.datatype = node["DataType"].asString();
            vssNode.length = node["Length"].asUInt();
            vssNode.methodId = node["MethodID"].asUInt();
            if (node.isMember("Transform")) 
            {
                const auto& transform = node["Transform"]["transform"];
                for (const auto& mapping : transform)
                {
                    std::string key = mapping["from"].asString();
                    const Json::Value& value = mapping["to"];
                    vssNode.transformMapping[key] = value;
                }
            }
            configinfo.vssNodes.push_back(vssNode);
        }
        
        vssInfo.push_back(configinfo);
    }
    return 0;
}
