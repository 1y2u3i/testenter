...
echo "Note: Light app will be creating the routing manager,
      it needs to be in running before starting other apps"
# Check if argument is provided
if [ $# -ne 1 ]; then
    echo -e "\e[31mError:App name is not provided ! \e[0m
      Usage: $0 <app name>
      eg: $0 'horn'"
    exit 1
fi

if [ -f "$SCRIPT_DIR/config/${1}_someip_feeder.json" ]; then
    VSOMEIP_CONFIGURATION="$SCRIPT_DIR/config/${1}_someip_feeder.json"
else
    GIT_ROOT=$(git rev-parse --show-toplevel)
    if [ -n "$GIT_ROOT" ] && [ -f "$GIT_ROOT/someip2val/config/${1}_someip_feeder.json" ]; then
        VSOMEIP_CONFIGURATION="$GIT_ROOT/someip2val/config/${1}_someip_feeder.json"
    fi
fi
if [ -f "$SCRIPT_DIR/config/${1}_feeder_mapping.json" ]; then
    FEEDER_CONFIGURATION="$SCRIPT_DIR/config/${1}_feeder_mapping.json"
fi
...
export VSOMEIP_APPLICATION_NAME="someip_${1}_feeder"
